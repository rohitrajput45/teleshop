   ...
   function getUser($table,$select="*",$where=array()){
	...
	//	$where['requestStatus'] 	=  0;
		...
		if($sql->num_rows()){
			...
			$shiftReport  	= $delivery->shiftReport;
			$deliveryDistance  	= 0;
			...
			//$subOrderId
			/* order info*/
				$orderSelect="contactName as customerName,phoneNumber as customerPhoneNumber,customerEmail,billingAddressType,billingAddress,poBoxNumber,city,zipcode,deliveryAddress,deliveryLatitude,deliveryLongitude,paymentMode,totalPrice as totalAmount,customerPay,paymentStatus as customerPaymentStatus";
				$orderInfo=$this->commonDetail('customerOrder',array('id'=>$orderId),$orderSelect);
				$orderInfo->deliveryDistance = $deliveryDistance;
				$orderInfo->customerPay = !empty($orderInfo->customerPay) ? json_decode($orderInfo->customerPay,true): new stdClass();
			/* order info*/
			/* pit info*/
			...
			/*responce*/
			if($shiftReport){
					$shiftReport = json_decode($shiftReport,true); 	
					$shiftReport = $this->shiftData($shiftReport); 	
				}
			...
			$responce->shiftReport 		= $shiftReport;
			/*responce*/
			//$responce = $delivery;
		}//End FUnction
		return $responce;
	...
	function  deliveryHistory($deliveryId,$orderId){
		$resdata = array();
	...
	//	echo $this->db->last_query();die;
		if($sql->num_rows()){
			$deliverys = $sql->result();
			foreach($deliverys as $d => $delivery):
				$responce = new stdClass();
				...
				/* order info*/
					...
					$orderInfo=$this->commonDetail('customerOrder',array('id'=>$orderId),$orderSelect);
					$orderInfo->deliveryDistance 	= $deliveryDistance;
					$orderInfo->cutomerImage 		= "";
					$orderInfo->customerPay 		= !empty($orderInfo->customerPay)?json_decode($orderInfo->customerPay,true):array();
					if(!empty($orderInfo->customerPay)){
					 $orderInfo->customerPay = $this->setPayData($orderInfo->customerPay);
					}
				/* order info*/
				/* pit info*/
				$pitSelect="companyName as pitName,phoneNumber as pitContact,address as pitAddress,latitude as pitLatitude ,longitude as pitLongitude";
				...
				$responce->pitInfo 			= $pitInfo;
				$responce->productInfo 		= $productInfo;
				$responce->deliverydate 	= date("d/m/Y",strtotime($deliverydate));
				$responce->shiftReport  	= $shiftReport ;
				$resdata[] = $responce;
				/*responce*/
				//$responce = $delivery;
	...
	function shiftData($data){
		...
		if(!empty($data)):
			...
			if(!empty($data[$i]['beforeDelivery']['image'])):
				$img = $data[$i]['beforeDelivery']['image'];
				$data[$i]['beforeDelivery']['image'] = base_url()."uploads/report/".$img;

			endif;
			if(!empty($data[$i]['afterDelivery']['image'])):
				$img = $data[$i]['afterDelivery']['image'];
				$data[$i]['afterDelivery']['image'] = base_url()."uploads/report/".$img;

			endif;
			if(!empty($data[$i]['deliveryReceipt']['image'])):
				$img = $data[$i]['deliveryReceipt']['image'];
				$data[$i]['deliveryReceipt']['image'] = base_url()."uploads/report/".$img;
