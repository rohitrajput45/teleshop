...
abstract class REST_Controller extends CI_Controller {
    ...
    public function __construct($config = 'rest')
    {
        ...
        if($this->config->item('rest_auth_required') && in_array( strtolower($this->uri->segment(2)), $this->config->item('rest_auth_required'))){
            ...
            if(!empty($userAuthData)){
                ...
                $responseArray = array('message'=>ResponseMessages::getStatusCodeMessage(101),'authToken'=>'','responseCode'=>300);

                $response = $this->generate_response($responseArray);

                $this->response($response,SERVER_ERROR);
