...
class Stripe{
	...
	function save_bank_account_id($holderName,$dob,$country,$currency,$routingNumber,$accountNo,$address,$postalCode,$city,$state,$ssnLast){
		...
		if ($success!=1)	{
			$responseArray = array('status'=>0,'message'=>$error[0]);
	...
	function addCardAccount($name,$number,$exp_month,$exp_year,$cvv){
		...
		if ($success!=1)	{
			...
			$responseArray = array('status'=>1,'message'=>$result['id']);
			return $responseArray;
...
*/
    ...
    function save_card_id($email = '',$token = ''){
		...
		if(isset($customer->id) && !empty($customer->id)){
			return array('status'=>1,'message'=>$customer->id);
		}else{
			return false;
		}
	}
...
*/
		...
		if ($success!=1)	{
			...
			return array('status'=>1,'message'=>$transfer->balance_transaction);
		}
