...
class Truck_model extends CI_Model {
	...
	function truckAgainstOrder($unitType, $quantity, $officeId, $ProductYD3Tonnage=0){
		...
		die("truck");*/
		if($unitType != 10){
			return $this->getTrucksByTonnage($quantity, $officeId," maxLoad_ton","asc");
		}else{
			$trucks = $this->getTruckList($officeId,"maxYD3Bed","asc");
...
/*print_r($trucks_required);
				...
				foreach($trucks as $truck){
					...
					if($deliveryQuantity<=$truck->maxYD3Bed ){
						...
						if ($deliveryWeight <= $truck->maxLoad_ton ) {
							...
							$temp_trucks = $this->getTrucksByTonnage($deliveryWeight,$officeId, "maxYD3Bed","asc");
						}
	...
	function getTrucksByTonnage($quantity, $officeId,$keyBy,$sortOrder){
		
		$trucks = $this->getTruckList($officeId,$keyBy,$sortOrder);
	...
	function getTruckList($officeId,$keyBy,$sortOrder){ 
		$this->db->select('id as truckId,maxLoad_ton,truckClassId,maxYD3Bed')->from('trucks');
		$this->db->where(array('status'=>1,'officeId'=>$officeId));
